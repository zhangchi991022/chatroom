# coding=utf-8
# 代码文件：chapter24/ChatRoom/qq_server.py
...
while True:
    try:
        ...
        logger.info('服务器接收客户端，1消息：{0}'.format(json_obj))
            ...
            if user is not None and user['user_pwd'] == userpwd:  # 登录成功
                ...
                for friend in friends:
                    ...
                    if fid in cinfo_userids:  # 用户登录
                        friend['online'] = '1'
                        for friinfo in clientlist:
                            if fid == friinfo[0]:
                                friend['address'] = friinfo[1]
        ...
        elif command == COMMAND_REG:
            userid = json_obj['user_id']
            logger.info(userid)
            userpwd = json_obj['user_pwd']
            username = json_obj['user_name']
            logger.debug('user_id:{0}  user_pwd:{1}  user_name:{2}'.format(userid, userpwd, username))

            dao = UserDao()
            user = dao.reg(userid, userpwd, username)
            logger.info(user)
            json_obj = user
            json_obj['result'] = user['result']
            logger.info('服务器发送用户成功，消息：{0}'.format(json_obj))
            json_str = json.dumps(json_obj)
            # 给客户端发送数据
            server_socket.sendto(json_str.encode(), client_address)
